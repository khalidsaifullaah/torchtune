...
"""
    ...
    file_path = f"{data_dir}/{subset}_{split}_{len(processed_dataset)}_cot.jsonl"
...
def main(args):
    ...
    if args.combine:
        non_cot_path = file_path.replace("_cot", "")
        cot_dataset = datasets.load_dataset("json", data_files={"_": file_path}, split="_")
        non_cot_dataset = datasets.load_dataset("json", data_files={"_": non_cot_path}, split="_")
        combined_dataset = datasets.concatenate_datasets([cot_dataset, non_cot_dataset])
        combined_dataset = combined_dataset.shuffle(seed=42)
        orig_size = len(cot_dataset)
        new_size = len(combined_dataset)
        new_path = file_path.replace(str(orig_size), str(new_size))
